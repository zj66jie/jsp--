...
//连接数据库操作
	...
	if(conn!=null){
		...
		PreparedStatement tmt = conn.prepareStatement("Insert into teacher(name,psd) values('" + user + "','" + password + "')");  
        int rst = tmt.executeUpdate();  
          if (rst != 0){  
               out.println("<script language='javascript'>alert('添加成功！');window.location.href='Guanshiadd.jsp?stid="+Stid+"';</script>");    
          }
		else{
			 out.println("<script language='javascript'>alert('添加失败，出现错误');window.location.href='Guanshiadd.jsp';</script>");
